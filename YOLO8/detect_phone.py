...
def sort_by_confidence(tuple_cord_points: list[CordsPoint]):
...
def get_phones_from_cords(image, tuple_cord_points: list[CordsPoint]):
...
class DetectPhone:
    ...
    def find_cord_for_phones(self, image) -> list[CordsPoint]:
